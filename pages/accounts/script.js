...
{	 
	...
	$(".generate-btn").click(function () 
	{
		...
		if (confirm("Do you really want to regenerate Temporary password to this Account?")) 
		{

			const currentRow= $(this).closest("tr");
			const id=currentRow.attr("id");
			const username = currentRow.find("span:eq(2)").text();
			const name = currentRow.find("span:eq(1)").text();


			if (currentRow.find("span:eq(3)").text()=="") 
			{
				var tempass='';
            ...
            for (let i = 1; i <= 8; i++) {
                ...
                tempass += str.charAt(char)
            }
            $.ajax({  
				type: 'POST',  
				url: 'regen_temp_pass.php', 
				data: {id:id,username:username,
				       name:name,tempass:tempass},
				success: function(response) {
					console.log("Response :"+response);
			...
			{
				alert("Account already have temporary password");
			}
	...
	$("#add-cashier-btn").click(function () {
		...
		else
		{
			$.ajax({  
				...
				url: 'add-cashier.php', 
				data: {name:name,username:username,
				       tempass:tempass,position:position},
				success: function(response) {
