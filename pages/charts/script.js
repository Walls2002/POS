$(document).ready(function () {
	...
	var year_chart=parseInt($(".year-select").val());
	...
	xmlhttp1.open("POST","sales-per-month.php?year="+year_chart,true);
	xmlhttp1.send();
		...
		if(this.readyState == 4 && this.status == 200) 
		{
			...
			mychart1=new Chart("myPlot", {
				...
				options: {
					...
					title: {
						display: true,
						text: "Sales Month of "+year_chart
					},
					scales: {
						yAxes: [{
							scaleLabel: {
		...
		if(this.readyState == 4 && this.status == 200) 
		{
			...
			mychart2=new Chart("myPlot2", {
				...
				options: {
					...
					title: {
						display: true,
						text: "Total Transaction per Month of "+year_chart
					},
					legend:{
						labels: {
							filter: (legendItem, data) => data.datasets[0].data[legendItem.index] != 0
	...
	xmlhttp2.open("POST","sales-per-year.php?year="+year_chart,true);
	xmlhttp2.send();
	})
