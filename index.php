...
<body>
	...
	if (isset($_POST['submit'])) {
		...
		if (!empty($username) && !empty($password)) {
			$sql = "SELECT * FROM account WHERE username='$username' AND (password='$password' OR temp_pass='$password')";
			...
			if ($count == 1 && $position == "admin") {
				...
				if ($temp_pass!="") 
				{
					header("refresh:1; url=pages/change_pass/index.php?username=".$username."&id=".$id."&name=".$name);
				}
				else
				{
					header("refresh:1; url=pages/home/cashier.php?name=".$name."&username=".$username);
				}
			} 
			else 
			{
